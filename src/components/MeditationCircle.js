...
export default function MeditationCircle({ 
  ...
  // Animated values
  const completionProgress = useRef(new Animated.Value(completed ? 1 : 0)).current;
  const scaleValue = useRef(new Animated.Value(1)).current;
  const longPressProgress = useRef(new Animated.Value(completed ? 1 : 0)).current;
  ...
  useEffect(() => {
    ...
    const completionAnimation = Animated.spring(completionProgress, {
      toValue: completed ? 1 : 0,
      ...springConfig,
    });
    const ringAnimation = Animated.spring(longPressProgress, {
      toValue: completed ? 1 : 0,
      damping: 15,
      stiffness: 150,
      mass: 1,
    ...
    if (completed) {
      scaleAnimation = Animated.spring(scaleValue, {
        toValue: 1.1,
        damping: 10,
        useNativeDriver: true,
      });
      scaleAnimation.start();
      scaleResetTimer = setTimeout(() => {
        scaleAnimation = Animated.spring(scaleValue, {
          toValue: 1,
          damping: 10,
          useNativeDriver: true,
    ...
    return () => {
      if (scaleResetTimer) clearTimeout(scaleResetTimer);
      scaleAnimation?.stop();
      completionAnimation.stop();
      ringAnimation.stop();
    };
  }, [completed, completionProgress, longPressProgress, scaleValue]);

  const completionWashOpacity = completionProgress.interpolate({
    inputRange: [0, 0.15, 1],
    outputRange: [0, 0.4, 1],
  });
  const completionWashScale = completionProgress.interpolate({
    inputRange: [0, 1],
    outputRange: [0.35, 1.05],
  });
  const progressRingOffset = longPressProgress.interpolate({
    inputRange: [0, 1],
    outputRange: [CIRCLE_CIRCUMFERENCE, 0],
  });
  const progressRingOpacity = longPressProgress.interpolate({
    inputRange: [0, 0.1, 1],
    outputRange: [0, 0.8, 1],
  });
          ...
          <View style={circleStyle}>
            ...
            <Animated.View
              style={[
                ...
                {
                  opacity: completionWashOpacity,
                  transform: [{ scale: completionWashScale }],
                },
              ]}
            />
